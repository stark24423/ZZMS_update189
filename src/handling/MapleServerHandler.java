...
//            System.err.println("[连结异常] " + cause.getMessage());
//            cause.getLocalizedMessage();
//            FileoutputUtil.printError("连结异常.txt", cause.getMessage());
//        }
//        if (!(cause instanceof IOException)) {
//            MapleClient client = (MapleClient) session.getAttribute(MapleClient.CLIENT_KEY);
//            if ((client != null) && (client.getPlayer() != null)) {
//                client.getPlayer().saveToDB(false, channel == MapleServerHandler.CASH_SHOP_SERVER);
//                FileoutputUtil.printError("连结异常.txt", cause, "连结异常 by: 角色:" + client.getPlayer().getName() + " 职业:" + client.getPlayer().getJob() + " 地图:" + client.getPlayer().getMap());
//            }
//        }
////        MapleClient client = (MapleClient) session.getAttribute(MapleClient.CLIENT_KEY);
...
//    }
    ...
    public void sessionOpened(final IoSession session) throws Exception {
        ...
        System.out.println("[连结线程] " + address + " 已连结");

        if (BlockedIP.contains(address)) {
            System.out.println("[登入服务] 已断开在禁止列表的客户端 IP:" + address + " ");
            session.close(true);
            return;
        }
        ...
        if (track == null) {
            ...
            if (count >= 20) {
                ...
                System.out.println("[登入服务] IP:" + address + " 连结次数超过限制断开连结");
        ...
        System.out.println("握手包发送到 " + address);
        Random r = new Random();
    ...
    public void sessionClosed(final IoSession session) throws Exception {
        ...
        if (client != null) {
            try {
                ...
                System.out.println("移除客户端 - " + finsh);
//                session.close(true);
        ...
        for (final RecvPacketOpcode recv : RecvPacketOpcode.values()) {
            if (recv.getValue() == header_num) {
                try {
                    ...
                    System.err.println(e.toString() +"封包错误");//
                    ZZMS.packet.println(slea.toString(true));
    ...
    public static void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c) throws Exception {
        switch (header) {
            case LOGIN_REDIRECTOR:
                if (!ServerConstants.REDIRECTION) {
            ...
            case CLIENT_HELLO:
                ...
                if ((mapleType != (ServerConstants.MAPLE_TYPE.getType())) || (mapleVersion != ServerConstants.MAPLE_VERSION) || (!maplePatch.equals(ServerConstants.MAPLE_PATCH))) {
                    c.getSession().close(true);
                    System.out.println(c.getSessionIPAddress() + " 用户端不正确,断开连接");
                } else {
                    System.out.println(c.getSessionIPAddress() + " 连接到游戏!");
                }
                break;
            case CLIENT_START:
...
//                System.err.println("[用户端 报错] 错误代码 " + errortype + " 类型 " + type_str + "\r\n\t[数据] 长度 " + data_length + " [" + SendPacketOpcode.nameOf(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
            ...
            case GET_SERVER:
                ...
                break;
            case GET_UNK:
                c.getSession().write(LoginPacket.getUNK());
                //c.getSession().write(LoginPacket.getPing());
//                c.sendPing();
                break;
            // END OF LOGIN SERVER
...
//                CashShopOperation.doCSPackets(c);
            ...
            case CHRONOSPHERE:
                ...
                break;
            case GUIDE_TRANSFER:
                PlayerHandler.GuildTransfer(slea, c, c.getPlayer());//游戏向导
                break;
            case ALLOW_PET_LOOT:
