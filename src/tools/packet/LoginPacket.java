...
//        mplew.write(ServerConstants.MAPLE_TYPE.getType());
        mplew.writeShort(ServerConstants.MAPLE_TYPE.getType());
        mplew.write(1);
        System.out.println("发送:" + HexTool.toString(mplew.getPacket()));
    ...
    public static final byte[] getUNK() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter(2);

        mplew.writeShort(SendPacketOpcode.GET_UNK.getValue());
        //是否有更新
        mplew.write(1);
    ...
    /**
     *
     * @param status 类型(status)： [1] 您所選擇的伺服器人數較多，建議您選擇其他伺服器來創建角色或進行遊戲！ [2]
    ...
    public static byte[] getCharList(String secondpw, List<Integer> charPos, List<MapleCharacter> chars, int charslots) {
        ...
        mplew.write(3);
        mplew.write((secondpw == null || secondpw.length() <= 0) ? 0 : 1);
        mplew.writeInt(charslots);
        System.out.print(charslots);
        mplew.writeInt(0);
        mplew.writeInt(-1);
        mplew.write(false);
